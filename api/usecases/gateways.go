...
type Database interface {
	...
	UpdateUser(ctx context.Context, address string, name *string, avatar *entities.Avatar) error
	CreateComment(ctx context.Context, threadId int64, address string, repliedToCommentId *int64, content string, imageFileName string, imageOriginalURL string, imageResizedURL string) (entities.Comment, error)
	CreateThread(ctx context.Context, address string, title string, content string, imageFileName string, imageOriginalURL string, imageResizedURL string) (entities.Thread, error)
...
type Images interface {
	...
	UploadAvatar(ctx context.Context, uri string, isNFT bool) (*entities.Avatar, error)
}
